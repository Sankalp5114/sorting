...
class Visualize extends Component {
  ...
  createArray = (size) => {
    ...
    for (let i = 0; i < size; i++) {
      arr.push({
        value: Math.floor(Math.random() * (170 - 30 + 1)) + 30, // Max value is set to 170
        id: 'id-' + i,
      });
    }
    this.setState({
  ...
  sortFunc = (e) => {
    ...
    if (method === 'Algorithms') {
      ...
      if (method === 'Bubble Sort') results = bubbleSort(arr, length);
      else if (method === 'Selection Sort') results = selectionSort(arr, length);
      else if (method === 'Merge Sort') results = mergeSort(arr, length);
      else if (method === 'Quick Sort') results = quickSort(arr, length);
      else if (method === 'Insertion Sort') results = insertionSort(arr, length);
      else if (method === 'Heap Sort') results = heapSort(arr, length);
  ...
  runSortingAnimation = () => {
    ...
    const intervalId = setInterval(() => {
      if (currentStep < results.length) {
        ...
        this.setState({ isSorting: false, currentStep: 0 });
      }
    }, speed);
      ...
      case 'Selection Sort':
        return (
          <>
            ...
            - Used in simple embedded systems where resource availability is limited and simplicity is important.<br/><br/>
